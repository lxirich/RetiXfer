...
class ClipAdapter(LanguageAdapterWrapper):
    def __init__(self, model, targets, domain_knowledge=False, tta=False, fta=False):
        ...
        print(f"Total parameters in self.adapter: {total_params}")
        print(f"Trainable parameters in self.adapter: {trainable_params}")
...
class TipAdapter(LanguageAdapterWrapper):
    ...
    def train(self, X, Y):
        ...
        if self.train_tip:
            ...
            total_params = sum(p.numel() for p in self.adapter_layer.parameters())
            trainable_params = sum(p.numel() for p in self.adapter_layer.parameters() if p.requires_grad)
            print(f"Total parameters in self.adapter_layer: {total_params}")
            print(f"Trainable parameters in self.adapter_layer: {trainable_params}")
